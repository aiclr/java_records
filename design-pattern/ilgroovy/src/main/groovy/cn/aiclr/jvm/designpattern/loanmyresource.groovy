...
/**
 * <pre>Loan(借出，贷与，出借) my Resource pattern
 * 出借我的资源模式
 */
...
/**
 * <pre>Groovy 使用了普通的 Java Reader 和 PrintWriter 对象
 * 代码编写器不必担心显式地创建或关闭这些资源
 * 内置的 Groovy 方法将相应的读取器或写入器借给闭包代码，然后自己整理
 */
def reader = f.newReader()
/**
 * <pre>处理文件中每一行的单词列表
 * 可以使用 Groovy 的内置函数来实现这一点
 * 也可以自己进行一些资源处理
 */
...
/**
 * <pre>Notice that we now have an explicit call to close() in our code.
