...
public class DealDeadLock {
     ...
     *
     * 线程 A 调用 a1 此时 线程 A 持有 al 锁,休眠 1 秒钟后，线程 A 访问资源 b 的 b2 方法 试图获取 bl 锁，
     * 线程 B 调用 b1 此时 线程 B 持有 bl 锁,休眠 1 秒钟后，线程 B 访问资源 a 的 a2 方法 试图获取 al 锁，
     * 此时
     * 线程 A 持有 al 锁，且等待 B 线程释放 bl 锁
     * 线程 B 持有 bl 锁，且等待 A 线程释放 al 锁
     *
     * 出现两个线程在互相等待释放资源的情况，也就是死锁
     * 但是 a2 b2 方法中 获取锁使用的是 {@link java.util.concurrent.locks.Lock#tryLock(long, java.util.concurrent.TimeUnit)}
     * 获取锁超时，不再继续尝试锁定资源。避免死锁
     */
    public static void main(String[] args) {
        ReentrantLock al = new ReentrantLock();
        DeadLockFooA a = new DeadLockFooA(al);

        ReentrantLock bl = new ReentrantLock();
        DeadLockFooB b = new DeadLockFooB(bl);
    ...
    /**
     * <pre>使用 {@link java.util.concurrent.locks.Lock#tryLock(long, java.util.concurrent.TimeUnit)} 自旋锁（Spin Lock），它跟互斥锁一样，
     * 如果一个执行单元要想访问被自旋锁保护的共享资源，则必须先得到锁，
     * 在访问完共享资源后，也必须释放锁。
     * 如果在获取自旋锁时，没有任何执行单元保持该锁，那么将立即得到锁；
     * 如果在获取自旋锁时锁已经有保持者，那么获取锁操作将“自旋”在那里，直到该自旋锁的保持者释放了锁为止。
     * 在我们的例子中就是线程 A 等待线程 B 释放锁，在 1 秒内不断尝试是否能够获得锁，
     * 达到 1 秒后还未获得锁资源，线程 A 则结束运行，线程 B 将获得资源继续执行，
     * 死锁解除
     */
    static class DeadLockFooA {
        ...
        public DeadLockFooA(Lock lock) {
            this.lock = lock;
        }

        public void a1(DeadLockFooB b) {
