 ...
 *      而细节就是实现类，实现接口或继承抽象类而产生的类就是细节，其特点是可以直接被实例化，也就是可以加上一个关键字 new 产生一个对象。
 *
 ...
 *      list 的表面类型为 List，是一个列表的抽象，而实际类型为 ArrayList
 *      在可能的情况下，尽量抽象表面类型，但也存在不必抽象的情况，比如工具类 xxxUtils，一般是不需要接口或是抽象类的。
 *  3）任何类都不应该从具体类派生
 ...
 *      为什么？
 *          维护工作基本上都是做扩展开发的，
 *          修复行为通过一个继承关系，覆写一个方法就可以修正一个很大的 Bug，
 ...
 *
 *  依赖抽象的优点在小型项目中很难体现出来，例如在小于 10 个人月的项目中，使用简单的 SSH 架构，基本上不用费太大力气就可以完成，是否采用抽象依赖原则影响不大。
