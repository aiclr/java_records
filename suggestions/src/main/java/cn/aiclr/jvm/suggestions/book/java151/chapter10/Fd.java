 ...
 *      2)缓慢时系统的 CPU、内存、I/O如何？
 *      3)高峰期和低谷时业务并发数量、并发交易种类、连接池的数量、数据的连接数量如何？
 *      4)最早接到用户投诉是什么时候，是如何处理的，优化后如何？
 *      5)数据量的增长幅度如何，是否有历史数据处理策略？
 *      6)系统是否有不稳定的情况，是否出现过宕机，是否产生过 宕机排查文件？
 *          Heap Dump	    heapdump.hprof	    不自动输出（需配置）	    内存泄漏分析
 *          GC Log	        gc.log	            不自动输出（需配置）	    GC 行为分析
 *          Thread Dump	    threaddump.log	    不自动输出（需手动/脚本）	死锁、阻塞分析
 *          HS Error Log	hs_err_pid12345.log	自动输出	                JVM 崩溃原因
 *          Core Dump	    core.12345	        不自动输出（需 ulimit）	深度调试（可选）
 *      7)最后一次变更是何时，变更的内容是哪些，变更后是否出现过性能问题？
 *      8)操作系统、网络、存储、应用软件等环境是否发生过改变？
 * 4）切
 *    要我们接触真实的系统数据，需要去看设计，看代码，看日志，看系统环境，然后是思考分析，最后给出结论。
 *    在这一环节中，需要注意两点：
 *       一是所有的非一手资料（如报告、非系统信息）都不是 100% 可信的，
