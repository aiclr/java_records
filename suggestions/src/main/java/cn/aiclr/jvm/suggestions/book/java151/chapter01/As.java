 ...
 * assert <布尔表达式>:<错误信息>
 *
 * 在布尔表达式为 false 时，抛出 AssertionError 错误，并附带错误信息
 * assert 的语法特性
 * 1. assert 默认是不启用的,需要设置 jvm 参数，参数是<code>-enableassertions</code>或者<code>-ea</code>
 *      断言是为了调式程序服务的，目的是为了能够快速、方便地检查程序异常，
 *      但 Java 在默认条件下是不启用的，要启用就需要在编译、运行时加上相关的关键字，
 * 2.assert 抛出的异常 AssertionError 是继承自 Error 的
 *      断言失败 JVM 会抛出一个 AssertionError 错误，是不可恢复的，也就表示这是一个严重问题，开发者必须予以关注并解决。
 *
 * assert 虽然是做断言的，但不能将其等价于 if...else
 *
 *  不可使用的情况
 ...
 *  2.在执行逻辑代码的情况下
 *      assert的支持是可选的，在开发时可以让它运行，但在生产系统中则不需要其运行（提高性能）因此，在 assert 的布尔表达式中不能执行逻辑代码，否则会影响环境不同而产生不同的逻辑。
 *  可用的情况：
 *  1.在私有方法中放置 assert 作为输入参数的校验
 *      在私有方法中可以放置 assert 校验输入参数，
 *      因为私有方法的使用者是作者自己，
 *      私有方法的调用者和被调用者之间是一种弱契约关系，或者说没有契约关系，
 *      其间的约束是依靠作者自己控制的，因此加上 assert 可以更好地预防自己犯错，或者无意的程序犯错
 *  2.流程控制中不可能达到的区域
 *      类似于 JUnit 的 fail 方法，其标志性的意义就是：程序执行到这里就是错误的
 *  3.建立程序探针
 *      我们可能会在一段程序中定义两个变量，分别代表两个不同的业务含义，
 *      但是两者有固定的关系，例如 <code>var1=var2*2;</code>，
 *      那我们就可以在程序中到处设 “桩”，断言这两者的关系，
 *      如果不满足即表明程序已经出现了异常，业务也就没有必要运行下去了
 */
public class As {
    /**
     * <pre>1.在对外公开的方法中 不能使用
     *
