 ...
 *
 * 一个类至少有一个构造函数（如果没有，编译器会为其创建一个无参构造函数）
 * 构造函数是在对象生成时调用的
 * 构造代码块不具有独立执行的能力，编译器会把构造代码块插入到每个构造函数的最前端 {@link Bj} 和 {@link BjA}相当
 ...
 *      例如在 J2EE 开发中，要产生 HTTP Request 必须首先建立 HTTP Session，
 *      在创建 HTTP Request 时就可以通过构造代码块来检查 HTTP Session 是否已经存在，不存在则创建之
 *
 * 构造代码块的两个特性：
 *      1.在每个构造函数中都运行
 *      2.在构造函数中它会首先运行
 ...
 * 这样在创建实例对象时 JVM 也就会按照顺序依次执行，实现复杂对象的模块化创建
 */
public class Bj {
    {
        //构造代码块3
        System.out.println("执行构造代码块3");
    }
