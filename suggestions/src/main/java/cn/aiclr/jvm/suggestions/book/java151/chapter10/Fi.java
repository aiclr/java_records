 ...
 *
 * 某些 Javaer 一直在质疑 Java 系统的性能，
 * 于是我们自己也跟着怀疑 Java 的性能 ----- 这就是发生在我们身边的真实“咕咚”，
 * Java 系统的性能问题本就是子虚乌有的事情，是我们自己吓唬自己，其实，我们可以从四个方面分析该问题：
 * 1）没有慢的系统，只有不满足业务的系统
 *      不管是使用 C 开发还是 Java 开发的项目，最终都会有一个产品诞生，或服务于大众（如网站），或服务于企业（企业级应用），
 ...
 *
 *      例如一个做统计的业务人员去看计费系统，即使响应需要 N 秒的时间，统计人员也会觉得非常快了，
 *      那是因为统计系统的结果经常是按照小时、天来计算的。
 *      再比如即时通信系统，有 1 秒内的延迟是可以接受的（发送者发出消息到接受者接收消息的时间间隔为 1 秒），
 *      但是语音通信系统若有 1 秒的延迟就是不可接受的了；
 *      发送邮件 N 分钟后才收到，这是可以容忍的，
 ...
 *
 *      比如有一个批处理系统，系统建设时的目标是：5 小时内生成 2000 万条业务数据，
 *      可到第 3 年的时候，公司发生了大规模的变化（整合了其他同类公司），
 *      需要处理的数据更多了，在 5 小时内需要生成 8000 万业务。于是就得考虑架构的扩展了。
 ...
 *      二是增强系统的处理能力，增大吞吐量，
 *          比如提升数据源的响应能力，
 *          划分数据的热度（如把数据划分为 Hot、Warm、Cold 等区域，分配不同的硬件资源和服务等级），
 ...
 *      曾经作者遇到一个运行超过 8 年的分析系统，
 *      从 1 年前开始只要是高峰期它的速度就会慢下来，
 *      分析下来，发现是因为并发用户超过了许可的数量，造成系统阻塞，性能缓慢，
 *      唯一解决的法就是购买更多的许可数量，
 *      但是 8 年了，一个系统的生命期还能有多少呢？
 ...
 *      性能问题也一样：
 *          我们可能不知道问题产生的原因，但我们可以有 N 种手段来解决它。能够解决的问题还算是问题吗？
 *
 */
public class Fi {
}