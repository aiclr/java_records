 ...
 *
 * 对于final修饰的基本类型和 String 类型，编译器会认为它是稳定态（ImmutableStatus），
 * 所以在编译时就直接把值编译到字节码中，
 * 避免了在运行期引用（Run-timeReference），以提高代码的执行效率。
 * 针对我们的例子来说，At类在编译时，字节码中就写上了 “150” 这个常量，而不是一个地址引用，
 * 因此无论你后续怎么修改常量类，只要不重新编译 Client 类，输出还是 “150”
 *
 * 对于 final 修饰的实例对象（即非基本类型），编译器认为它是不稳定态（Mutable Status），
 * 在编译时建立的是引用关系（该类型也叫做 Soft Final），
...
public class At {
    public static void main(String[] args) {
        System.out.println(AtConstant.MAX_AGE);
    }
}
