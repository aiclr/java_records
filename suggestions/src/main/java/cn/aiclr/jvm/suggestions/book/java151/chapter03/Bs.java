package cn.aiclr.jvm.suggestions.book.java151.chapter03;

import java.util.ArrayList;
import java.util.List;

/**
 * <pre>45.覆写equals方法时不要识别不出自己
 */
public class Bs {

    public static void main(String[] args) {
        PersonBs p1 = new PersonBs("张三");
        PersonBs p2 = new PersonBs("张三 ");
        List<PersonBs> l = new ArrayList<>();
        l.add(p1);
        l.add(p2);
        System.out.println(l.contains(p1));
        System.out.println(l.contains(p2));
    }
    
}

class PersonBs {
    private String name;

    public PersonBs(String _name) {
        name = _name;
    }

    /**
     * 违反了 equals 的自反性原则：对于任何非空引用 x，x.equals(x) 应该返回 true.
     * 解决也非常容易，只要把trim()去掉即可
     * 注意解决的只是当前问题，该 equals 实现还存在其他问题
     */
    @Override
    public boolean equals(Object obj) {
        if (obj instanceof PersonBs p) {
            return name.equalsIgnoreCase(p.getName().trim());
        }
        return false;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }
}
