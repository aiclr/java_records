...
/**
 * <pre>63.在明确的场景下，为集合指定初始容量
 *
 * 注意：非常有必要在集合初始化时声明容量
 *
 * Vector 与 ArrayList 不同的地方是它提供了递增步长（capacityIncrement变量），
 * 其值代表的是每次数组拓长时要增加的长度，
 * 不设置此值则是容量翻倍（默认是不设置递增步长的，可以通过构造函数来设置递增步长）。
 * 其他集合类的扩容方式与此相似，如 HashMap 是按照倍数增加的，
 * Stack 继承自 Vector，所采用的也是与其相同的扩容原则
 *
 * ArrayList、Vector、HashMap 等集合，一般都是直接用 new 跟上类名声明出一个集合来，然后使用 add、remove 等方法进行操作,而且因为它是自动管理长度的
 * jdk21 源码：
 *      {@link ArrayList#add(Object)} -> {@link ArrayList#ensureCapacity(int)} -> {@link ArrayList#grow()}
 * jdk8 源码
 *      ArrayList add 方法 -> ensureCapacityInternal 方法 -> ensureExplicitCapacity 方法
 *      -> grow 方法计算新的集合长度，使用移位运算，扩大约1.5倍
