 ...
 * 并且还需要限定并发数、资源数（如 10 万数据和 1000 万的数据响应时间肯定不同）等，
 ...
 *          对技术人员来说，一个请求在 2 秒钟之内响应就可以认为是“非常快”了，
 *          但对业务人员来说，“非常快”指的是在 0.5 秒内看到结果
 *      看: 出现偏差了。
...
public class Ff {
    ...
    private static final Logger logger = LoggerFactory.getLogger(Ff.class);

    /**
     * <pre>移位运算性能确实提高了，但是也带来了副作用，
     * 比如代码的可读性降低了很多，
    ...
    public static void main(String[] args) {
        ...
        logger.info("左移运算");
        logger.info("{} << {} = {}", 100, 1, 100 << 1);//200
        logger.info("{} << {} = {}", 100, 2, 100 << 2);//400
        logger.info("{} << {} = {}", 100, 3, 100 << 3);//800
        logger.info("{} << {} = {}", 100, 4, 100 << 4);//1600
        logger.info("{} << {} = {}", 111, 1, 111 << 1);//222
        logger.info("{} << {} = {}", 111, 2, 111 << 2);//444
        logger.info("{} << {} = {}", 111, 3, 111 << 3);//888
        logger.info("{} << {} = {}", 111, 4, 111 << 4);//111*16
        logger.info("{} << {} = {}", -111, 2, -111 << 2);//-111*16

        //右移，左边空出的位用0或者1填补。正数用0填补，负数用1填补。
        // 注：不同的环境填补方式可能不同；低位右移溢出则舍弃该位
        logger.info("有符合右移运算");
        logger.info("{} >> {} = {}", 100, 1, 100 >> 1);//50
        logger.info("{} >> {} = {}", 50, 1, 50 >> 1);//25
        logger.info("{} >> {} = {}", 25, 1, 25 >> 1);//12
        logger.info("{} >> {} = {}", 12, 1, 12 >> 1);//6
        logger.info("{} >> {} = {}", 100, 2, 100 >> 2);//25
        logger.info("{} >> {} = {}", 100, 3, 100 >> 3);//12
        logger.info("{} >> {} = {}", 100, 4, 100 >> 4);//6
        logger.info("{} >> {} = {}", -100, 2, -100 >> 2);//-25
        logger.info("{} >> {} = {}", -100, 4, -100 >> 4);//-7
        logger.info("{} / {} = {}", -100, 16, -100 / 16);//-6
        logger.info("{} % {} = {}", -100, 16, -100 % 16);//-4 余

        //无符号右移
        //正数与右移规则一样，负数的无符号右移，就是相应的补码移位所得，在高位补0即可
        logger.info("无符号右移运算");
        logger.info("{} >>> {} = {}", 100, 1, 100 >>> 1);
        logger.info("{} >>> {} = {}", -100, 1, -100 >>> 1);
    }
}
