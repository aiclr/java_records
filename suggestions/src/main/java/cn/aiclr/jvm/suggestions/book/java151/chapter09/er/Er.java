...
/**
 * <pre>122.使用线程异常处理器提升系统可靠性
 * Java 1.5 版本以后增加了 {@link java.lang.Thread#setUncaughtExceptionHandler(java.lang.Thread.UncaughtExceptionHandler)} 方法，实现了线程异常的捕捉和处理
 ...
 *      异常的产生中断了正在执行的业务逻辑，
 *      特别是如果正在执行一个原子操作
 *         （像即时通信服务器的用户验证和签到这两个事件应该在一个操作中处理，不允许出现验证成功但签到不成功的情况），
 ...
 *      但是如果线程反复重启，很可能会引起 OutOfMemory 内存泄露问题
 */
public class Er {
    public static void main(String[] args) {
        TcpServer tcpServer = new TcpServer(3);
    }
}
