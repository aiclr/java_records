...
public class Bh {
     ...
     *
     * {@code ServerBh serverBh = new SimpleServerBh(1000);}代码执行过程：
     * 子类 SimpleServerBh 的构造函数接收 int 类型的参数：1000
     * 父类初始化常量，也就是 DEFAULT_PORT 初始化，并设置为 40000
     * 执行父类无参构造函数，也就是子类的有参构造中默认包含的 super() 方法
     * 父类无参构造函数执行到 {@code int port=getPort();} 方法，调用子类的 getPort 方法实现
     * 子类的 getPort 方法返回 port 值（注意，此时 port 变量还没有赋值，是0）或 DEFAULT_PORT（此时已经是40000）了
     * 父类初始化完毕，开始初始化子类的实例变量，port 赋值 100
     * 执行子类构造函数，port 被重新赋值为 1000
     * 子类 SimpleServerBh 实例化结束，对象创建完毕
     */
...
abstract class ServerBh {
    ...
    public ServerBh() {
        int port = getPort();
        System.out.println("端口号：" + port);//40000或者0永远不会出现100或者1000
    }
