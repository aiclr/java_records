 ...
 * <pre>62.警惕数组的浅拷贝
 * 案例：
 *  第一个箱子里有赤、橙、黄、绿、青、蓝、紫、7色气球，
 *  现在希望在第二个箱子中也放入7个气球，其中最后一个气球改为蓝色，也就是赤、橙、黄、绿、青、蓝、蓝、7个气球，
...
public class Cj {
    ...
    public static void main(String[] args) {
        ...
        for (BalloonCj balloonCj : box1) {
            System.out.println(balloonCj);
        }

        System.out.println("----------------------");
        BalloonCj[] box2 = Arrays.copyOf(box1, box1.length);
        /**
         * box1[6] 颜色也会被改变，因为数组拷贝 copyOf() 方法是浅拷贝，
         * 与序列化的浅拷贝完全相同：
         *      基本类型是直接拷贝值
         *      其他都是拷贝引用地址
         * 数组的 clone 方法也是与此相同的，同样是浅拷贝，
         * 而且集合的 clone 方法也都是浅拷贝
         */
