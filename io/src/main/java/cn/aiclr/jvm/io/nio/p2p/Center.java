...
public class Center {
    ...
    public static void main(String[] args) {
        ...
        try (ServerSocketChannel serverSocketChannel = ServerSocketChannel.open();
            ...
            logger.info("===>{}", serverSocketChannel.hashCode());
            while (true) {
                //等待2秒，如果没事件返回
                if (selector.select(2000) == 0) {
                    logger.info("服务器等待2秒无连接");
                    continue;
                }
                ...
                while (keyIterator.hasNext()) {
                    ...
                    logger.info("有事件发生 selectionKey hashCode=[{}],readyOps=[{}],interestOps=[{}]", selectionKey.hashCode(), selectionKey.readyOps(), selectionKey.interestOps());
                    ...
                    if (selectionKey.isAcceptable()) {
                        ...
                        logger.info("get connection from {}", socketChannel);
                    }
                    //读事件发生
                    if (selectionKey.isReadable()) {
                        ...
                        try {
                             ...
                             */
                            int read = socketChannel.read(byteBuffer);//position 会移动 客户端断开 会抛 IOException 异常
                            ...
                            if (read == -1) {
                                ...
                                logger.info("get data from {}", socketChannel);
                                socketChannel.register(selector, SelectionKey.OP_WRITE, byteBuffer);
                                // 在 OP_READ 中处理完读取后，触发对所有客户端的写操作
                                Set<SelectionKey> keys = selector.keys();
                                for (SelectionKey key : keys) {
                                    if (key.channel() instanceof SocketChannel && key.isValid() && key.interestOps() != SelectionKey.OP_WRITE) {
                    ...
                    if (selectionKey.isWritable()) {
                        ...
                        //将生成的socketchannel注册到selector,设置为OP_READ事件，绑定buffer 之后从客户端读取数据
                        socketChannel.register(selector, SelectionKey.OP_READ, byteBuffer);
                        logger.info("send data to {}", socketChannel);
                    }
                    keyIterator.remove();
                }
            }
